/*
	UserPlugIn.h File Generated by Socket
...
void CResonantLPF::calculateLPFCoeffs(float fCutoffFreq, float fQ)
{
	...
	if(m_uType == Mass1)
	{
		calculateAnalogModelLPFCoeffs(fCutoffFreq, fQ, 1);
		return;
	}

	if(m_uType == Mass2)
	{
		calculateAnalogModelLPFCoeffs(fCutoffFreq, fQ, 2);
		return;
	}

	if(m_uType == Rez1)
	{
		calculateResonatorCoeffs(fCutoffFreq, fQ);
		return;
	}

	if(m_uType == Rez2)
	{
		calculateImprovedResonatorCoeffs(fCutoffFreq, fQ);
		return;
	}

	if(m_uType == MMALPF)
	{
		calculateMMALPFCoeffs(fCutoffFreq, fQ);
		return;
	}
	...
	if(m_uType == B2L)
	{
		calculateB2LPFCoeffs(fCutoffFreq, fQ);
		return;
	}

	if(m_uType == B2H)
	{
		calculateB2HPFCoeffs(fCutoffFreq, fQ);
		return;
	}

	if(m_uType == B2BPF)
	{
		calculateB2BPFCoeffs(fCutoffFreq, fQ);
		return;
	}
	if(m_uType == B2BSF)
	{
		calculateB2BSFCoeffs(fCutoffFreq, fQ);
		return;
	}
	if(m_uType == Param)
	{
		calculateParametricCoeffs(fCutoffFreq, fQ);
		return;
	}
	if(m_uType == ConstQ)
	{
		calculateConstQCoeffs(fCutoffFreq, fQ);
		return;
	}
	...
	if(m_uType == DigiLP1)
	{
		...
		m_LeftLPF.m_f_a0 = a0;
		m_LeftLPF.m_f_a1 = a1;
		m_LeftLPF.m_f_a2 = 0.0;
		m_LeftLPF.m_f_b1 = b1; 
		m_LeftLPF.m_f_b2 = 0.0;

		// right channel
		m_RightLPF.m_f_a0 = a0;
		m_RightLPF.m_f_a1 = a1;
		m_RightLPF.m_f_a2 = 0.0;
		m_RightLPF.m_f_b1 = b1; 
		m_RightLPF.m_f_b2 = 0.0;
...
void CResonantLPF::calculateConstQCoeffs(float fCutoffFreq, float fQ)
{
	float K = tanf(pi*fCutoffFreq/(float)m_nSampleRate);

	float Vo = pow(10.0, m_fBoostCut/20.0);

	bool bBoost = m_fBoostCut >= 0 ? true : false;

	//if(!bBoost)
	//	fQ /= 4.5;

	float d0 = 1.0 + (1.0/fQ)*K + K*K;
	float e0 = 1.0 + (1.0/(Vo*fQ))*K + K*K;
	
	float alpha = 1.0 + (Vo/fQ)*K + K*K;
	float beta = 2.0*(K*K - 1.0);
	float gamma = 1.0 - (Vo/fQ)*K + K*K;
	float delta = 1.0 - (1.0/fQ)*K + K*K;
	float eta = 1.0 - (1.0/(Vo*fQ))*K + K*K;
...
void CResonantLPF::calculateParametricCoeffs(float fCutoffFreq, float fQ)
{
	float theta_c = 2.0*pi*fCutoffFreq/(float)m_nSampleRate;
	...
	float nose = 4.0/(1.0 + mu);
	
	// intermediate values
	float fBetaNumerator =   1.0 - nose*tan(tanArg);
	float fBetaDenominator = 1.0 + nose*tan(tanArg);
	...
	// alpha
	//float fAlpha = (0.5 - fBeta)/2.0;
	float fAlpha = (0.5 - fBeta);

	//// beta
	//float fBeta = (fBetaNumerator/fBetaDenominator);
	//
	//// gamma
	//float fGamma = (1 + fBeta)*(cos(theta_c));

	//// alpha
	//float fAlpha = (1 - fBeta);
...
void CResonantLPF::calculateB2LPFCoeffs(float fCutoffFreq, float fQ)
{
	// use same terms as in book:
	float theta_c = pi*fCutoffFreq/(float)m_nSampleRate;
	
	float C = 1.0/tan(theta_c);

	float a0 = 1.0/(1.0 + pow(2.0, 0.5)*C + C*C);
	float a1 = 2.0*a0;
	float a2 = a0;
	float b1 = 2.0*a0*(1.0 - C*C);
	float b2 = a0*(1.0 - pow(2.0, 0.5)*C + C*C);
...
void CResonantLPF::calculateB2HPFCoeffs(float fCutoffFreq, float fQ)
{
	// use same terms as in book:
	float theta_c = pi*fCutoffFreq/(float)m_nSampleRate;
...
void CResonantLPF::calculateB2BPFCoeffs(float fCutoffFreq, float fQ)
{
	float theta_c = 2.0*pi*fCutoffFreq/(float)m_nSampleRate;
	float BW = fCutoffFreq/fQ;
	float delta_c = pi*fCutoffFreq*BW/(float)m_nSampleRate;

	float C = 1.0/tan(delta_c);
	float D = 2.0*cos(theta_c);
...
void CResonantLPF::calculateB2BSFCoeffs(float fCutoffFreq, float fQ)
{
	float theta_c = 2.0*pi*fCutoffFreq/(float)m_nSampleRate;
	float BW = fCutoffFreq/fQ;
	float delta_c = pi*fCutoffFreq*BW/(float)m_nSampleRate;
...
void CResonantLPF::calculateBPFCoeffs(float fCutoffFreq, float fQ)
{
	// use same terms as in book:
	float theta_c = 2.0*pi*fCutoffFreq/(float)m_nSampleRate;
	
	// intermediate values
	float fBetaNumerator =   1.0 - tan(theta_c/2.0*fQ);
	float fBetaDenominator = 1.0 + tan(theta_c/2.0*fQ);

	m_fMeterValue = tan(theta_c/2.0*fQ);

	char *f = floatToString(theta_c/2.0*fQ, 2);
	char* p = addStrings("T/2Q = ", f);
	//this->sendStatusWndText(p);
	...
	// left channel
	m_LeftLPF.m_f_a0 = 0.5 - fBeta;
	m_LeftLPF.m_f_a1 = 0.0;
	m_LeftLPF.m_f_a2 = -1.0*(0.5 - fBeta);
	m_LeftLPF.m_f_b1 = -2.0*fGamma; 
	m_LeftLPF.m_f_b2 = 2.0*fBeta;

	// right channel
	m_RightLPF.m_f_a0 = 0.5 - fBeta;
	m_RightLPF.m_f_a1 = 0.0;
	m_RightLPF.m_f_a2 = -1.0*(0.5 - fBeta);
	m_RightLPF.m_f_b1 = -2.0*fGamma; 
	m_RightLPF.m_f_b2 = 2.0*fBeta;
}
	
void CResonantLPF::calculateBSFCoeffs(float fCutoffFreq, float fQ)
{
	// use same terms as in book:
	float theta_c = 2.0*pi*fCutoffFreq/(float)m_nSampleRate;
	...
	// left channel
	m_LeftLPF.m_f_a0 = 0.5 + fBeta;
	m_LeftLPF.m_f_a1 = -2.0*fGamma;
	m_LeftLPF.m_f_a2 = 0.5 + fBeta;
	m_LeftLPF.m_f_b1 = -2.0*fGamma; 
	m_LeftLPF.m_f_b2 = 2.0*fBeta;

	// right channel
	m_RightLPF.m_f_a0 = 0.5 + fBeta;
	m_RightLPF.m_f_a1 = -2.0*fGamma;
	m_RightLPF.m_f_a2 = 0.5 + fBeta;
	m_RightLPF.m_f_b1 = -2.0*fGamma; 
	m_RightLPF.m_f_b2 = 2.0*fBeta;
}

void CResonantLPF::calculateMMALPFCoeffs(float fCutoffFreq, float fQ)
{
	float fTheta = (2*pi*fCutoffFreq)/(float)m_nSampleRate;

	float fResdB = 0;
	if(fQ > 0.707)
	{
		float peak = fQ*fQ/pow(fQ*fQ - 0.25, 0.5);
		fResdB = 20.0*log10(peak);
	}

	float fRes = (cos(fTheta) + (sin(fTheta) * sqrt( pow(10.0,(fResdB/10.0))-1) ) )
			/ (
			(pow(10.0, (fResdB/20.0)) * sin(fTheta)) + 1
				);

	float fg = pow(10.0, (-fResdB/40.0));

	float b1 = (-2) * fRes * cos(fTheta);
	float b2 = fRes * fRes;
...
void CResonantLPF::calculateResonatorCoeffs(float fCutoffFreq, float fQ)
{
	float thetaC = (2.0*pi*fCutoffFreq)/(float)m_nSampleRate;

	float BW = fCutoffFreq/fQ;

	//float R = 1.0 - pi*BW/(float)m_nSampleRate;
	//float theta = acos(((2.0*R)/(1+R*R))*cos(thetaC));
	//float a0 = (1.0 - R*R)*sin(theta);
	...
	////float theta = acos(((2.0*R)/(1+R*R))*cos(thetaC));
	//float a0 = (1.0 - R);
	//float b1 = -2.0*R*cos(thetaC);
	//float b2 = R*R;

	float b2 = exp(-2.0*pi*BW/(float)m_nSampleRate);
	float b1 = ((-4.0*b2)/(1+b2))*cos(thetaC);
	float a0 = (1.0 - b2)*pow((1-(b1*b1)/(4.0*b2)), 0.5);
...
void CResonantLPF::calculateImprovedResonatorCoeffs(float fCutoffFreq, float fQ)
{
	float thetaC = (2.0*pi*fCutoffFreq)/(float)m_nSampleRate;
	...
	//float R = 1.0 - pi*BW/(float)m_nSampleRate;
	//float theta = acos(((2.0*R)/(1+R*R))*cos(thetaC));
	////float a0 = 0.25*pow((1.0 - R*R)*sin(theta), 0.5);
	//float a0 = (1.0 - R*R)*sin(theta);
	////float a0 = ((1.0 - R*R)/2.0)*(1.0 - R*R)*sin(theta);
	//float b1 = -2.0*R*cos(theta);
	//float b2 = R*R;
	...
	//float b2 = exp(-2.0*pi*(BW/(float)m_nSampleRate));
	//float b1 = (-4.0*b2/(1+b2))*cos(thetaC);
	//float a0 = 1.0 - pow(b2, (float)0.5);
	//float a2 = 1.0 - pow(b2, (float)0.5);
...
void CResonantLPF::calculateAnalogModelLPFCoeffs(float fCutoffFreq, float fQ, int nOrder)
{
	// use same terms as in book:
	float omegaC = 2.0*pi*fCutoffFreq/(float)m_nSampleRate;

	//float m = pow((pow(2.0, 0.5)*(float)m_nSampleRate)/fCutoffFreq, 2.0);
	//float n = pow((float)m_nSampleRate/(fQ*fCutoffFreq), (float)2.0);
	//float denom = pow(pow(2.0 - m, 2.0) + n, 0.5);

	if(nOrder == 1)
	{
		float g1 = 2.0/pow((4.0 + pow((float)m_nSampleRate/fCutoffFreq,2)),0.5);

		float gm = fmax(pow((float)0.5, (float)0.5), pow(g1, (float)0.5));

		float wm = (2.0*pi*fCutoffFreq*pow(1 - gm*gm, (float)0.5))/gm;

		float Omega_m = tan(wm/(2.0*(float)m_nSampleRate));
...
/* init()
	Called by the client when the plug-in is loaded; this is where you initialize
	sub-component and sub-objects
*/

bool __stdcall CResonantLPF::init()
{
	// Add your code here:
...
/* pause()
	Called by the client when the pause button is hit
	Generally, there is nothing to do here unless your plugin does really
	strange things due to a pause event.
*/

bool __stdcall CResonantLPF::pause()
{
	return true;
}
...
bool __stdcall CResonantLPF::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
	...
	// (INSERT Effect)
		...
		//m_LeftLPF.m_f_a0 = 1.0;
		//m_LeftLPF.m_f_a1 = 0.73;
		//m_LeftLPF.m_f_a2 = 1.0;
		//m_LeftLPF.m_f_b1 = -0.78;
		//m_LeftLPF.m_f_b2 = 0.88;

		//// right channel
		//m_RightLPF.m_f_a0 = 1.0;
		//m_RightLPF.m_f_a1 = 0.73;
		//m_RightLPF.m_f_a2 = 1.0;
		//m_RightLPF.m_f_b1 = -0.78;
		//m_RightLPF.m_f_b2 = 0.88;
...
{
	// ADDED BY SOCKET -- DO NOT REMOVE: Request ownership of the critical section.
//    EnterCriticalSection(&m_CriticalSection);

	// decode the control index, or delete the switch and use brute force calls
	switch(nControlIndex)
	{
		// for either control, calc the coeffs
		case 0:
		case 1:
		{
			calculateLPFCoeffs(m_f_fc_Hz, m_f_Q);
    ...
    // ADDED BY SOCKET -- DO NOT REMOVE: Release ownership of the critical section.
...
*/
bool __stdcall CResonantLPF::processSocketAudioBuffer(float* pInputBuffer, float* pOutputBuffer,
...
// **--0xDEA7--**
	...
	m_fBoostCut = 0.000000;
	CUICtrl ui2;
	ui2.uControlType = FILTER_CONTROL_CONTINUOUSLY_VARIABLE;
	ui2.uControlId = 2;
	ui2.bLogSlider = false;
	ui2.bExpSlider = false;
	ui2.fUserDisplayDataLoLimit = -12.000000;
	ui2.fUserDisplayDataHiLimit = 12.000000;
	...
	ui3.fUserDisplayDataHiLimit = 14.000000;
	...
	ui3.m_pUserCookedUINTData = &m_uType;
	ui3.cControlUnits = "Units                                                           ";
	ui3.cVariableName = "m_uType";
	ui3.cEnumeratedList = "DigiLP1,DigiLP2,Mass1,Mass2,MMALPF,Rez1,Rez2,BPF,BSF,B2L,B2H,B2BPF,B2BSF,Param,ConstQ";
	...
	m_fMeterValue = 0.0;
	CUICtrl ui4;
	ui4.uControlType = FILTER_CONTROL_LED_METER;
	ui4.uControlId = 59;
	...
	ui4.bUseMeter = true;
	ui4.bUpsideDownMeter = false;
	ui4.bLogMeter = false;
	ui4.uDetectorMode = 0;
	ui4.uMeterColorScheme = 0;
	ui4.fMeterAttack_ms = 10.000000;
	ui4.fMeterRelease_ms = 500.000000;
	ui4.cMeterVariableName = "m_fMeterValue";
	ui4.m_pCurrentMeterValue = &m_fMeterValue;
	ui4.nGUIRow = -1;
	ui4.nGUIColumn = -1;
