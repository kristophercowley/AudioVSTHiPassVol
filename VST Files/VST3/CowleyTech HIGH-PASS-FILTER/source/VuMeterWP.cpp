...
: CVuMeter(size, onBitmap, offBitmap, nbLed, style)
{
	...
	CCoord ht = getOnBitmap()->getHeight();
	CCoord ht2 = getOffBitmap()->getHeight();

	CCoord wd = getOnBitmap()->getWidth();
	CCoord wd2 = getOffBitmap()->getWidth();

	// --- the +1 is a fix for drawbug that 
	//     is probably from rounding in VSTGUI
	//
	// This code forces the on/off rect sizes to match the bitmap height + 1 (vert) or width + 1 (horiz)
	//if(style & kHorizontal) 
	//{
	//	rectOn.right = rectOn.left + wd + 1;
	//	rectOff.right = rectOff.left + wd + 1;
	//}
	//else
	//{
	//	rectOn.bottom = rectOn.top + ht + 5;
	//	rectOff.bottom = rectOff.top + ht2 + 5;
	//}
}
...
void CVuMeterWP::setViewSize(const CRect& newSize, bool invalid)
{
	...
	// --- added this inflation for VSTGUI issue when LED Meter has a border
	//if(style & kHorizontal) 
	//{
	//	rectOn.right += 1;
	//	rectOff.right += 1;
